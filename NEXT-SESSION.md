...
## Test the Plan Feature Pipeline

### 1. Re-test fetch-feature.ps1
```powershell
.\scripts\plan-feature\fetch-feature.ps1 -FeatureId 744719 -Org "wtw-bda-outsourcing-product" -Project "BenefitConnect"
```
...
### 2. Re-run refine-feature skill
Feed the fetched Feature JSON to the refine-feature skill interactively. Answer refinement questions until it produces a slice-ready summary.
...
```powershell
.\scripts\plan-feature\post-stories.ps1 -InputFile output/744719/stories.json -ParentId 744719 -Org "wtw-bda-outsourcing-product" -Project "BenefitConnect" -DryRun
```
...
### 7. End-to-end Plan Feature (real post to ADO)
...
- Dry-run refine-story with the fetched story; confirm it converges to a task DAG matching `schemas/plan-output.schema.json`
...
### 9. Wire TDD Skill into plan-story
- Add an explicit reference/invocation of the existing `tdd` skill in `.github/agents/plan-story.agent.md` (currently absent)
- Confirm task breakdown from refine-story (the `plan.json` task list) feeds into the TDD loop one task at a time, respecting the dependency DAG
