...
$item = az boards work-item show --id $StoryId --org $orgUrl --project $Project --output json | ConvertFrom-Json
...
if ($item.relations) {
    ...
    if ($attachment) {
        $briefContent = Invoke-RestMethod -Uri $attachment.url -Headers @{ Authorization = "Bearer $(az account get-access-token --query accessToken -o tsv)" }
    }
}
