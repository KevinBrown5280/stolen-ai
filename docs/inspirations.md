...
|--------|----------------------|----------------|
| Burke Holland | Orchestrator never implements; sub-agent delegation; parallel phases | `dev-workflow.agent.md`, `po-workflow.agent.md` |
| Matt Pocock | Grill-first pattern; TDD skill structure; write-a-skill conventions | `po-grill`, `dev-grill`, `tdd` skills |
